    ...
    return Controller.extend("ventilado.ventilado.controller.Log", {
        ...
        ejecutarAcciones: async function () {
            ...
            try {
                ...
                const totalCantidadAEsc = this.datosD.reduce((total, item) => total + (item.CantEscaneada || 0), 0);
...
oLocalModel.setProperty("/tableData", tableData); 
        ...
        onEdit: function (oEvent) {
            ...
            var oDialog = new sap.m.Dialog({
                title: 'Edite la cantidad Ingresada',
                content: [
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        request.onsuccess = function (event) {
                            ...
                            ctx.actualizarCantEscaneada(db, oData.Id, Number(updatedValue),resultadoFormateadoKgbrr , resultadoFormateadoM3r);
                            // obtener el valor actualizado
                           
                            // Salvar cambios y cerrar
                            var updatedData = [{ "Id": oData.Id, "CantEscaneada": Number(updatedValue) ,"Kgbrr":resultadoFormateadoKgbrr , "M3r":resultadoFormateadoM3r}];
        ...
        actualizarCantEscaneada: function (db, id, nuevaCantidadEscaneada,Kgbrr , M3r) {
            ...
            request.onsuccess = function (event) {
                ...
                if (data) {
                    ...
                    updateRequest.onsuccess = function () {
                        console.log("Cantidad Escaneada actualizada con éxito en IndexedDB.");
                    };
                    updateRequest.onerror = function (event) {
                        console.error("Error al actualizar Cantidad Escaneada en IndexedDB:", event);
                    };
                }
            };
        ...
        onDelete: function (oEvent) {
            ...
            sap.m.MessageBox.confirm("¿Está seguro que quiere borrar este ítem?", {
                onClose: function (oAction) {
                    if (oAction === sap.m.MessageBox.Action.OK) {
                        // Remove the item from the model
                        var oModel = this.getView().getModel();
                        // el borrado consiste en actualizar los campos que indican que se escaneo
                        // Salvar cambios y cerrar
                        var updatedData =[{ "Id": oData.Id,  "CantEscaneada": 0 ,"AdicChar1" :'' , "AdicChar2": '' , "AdicDec1":'', "AdicDec2":'' , "Preparador": '',  "Estado":'INICIAL'}] ;
                        ctx.actualizarBackEnd(updatedData); 
                        var request = indexedDB.open("ventilado", 5);
                        request.onsuccess = function (event) {
                            var db = event.target.result;
                            ctx._dbConnections.push(db); // Guardar referencia a la conexión abierta
                            // Llamar a la función para actualizar el campo 'CantEscaneada'
